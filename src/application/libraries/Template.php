...
class Template
{
	...
	public function build($view, $data = array(), $return = FALSE)
	{
		...
		$template['meta_k']	= $this->_meta_k;
		$template['meta_d']	= $this->_meta_d;
		$template['h_page']	= $this->_h_page;
		$template['h_site']	= $this->_h_site;
	...
	public function meta_keywords()
	{
		...
		if (func_num_args() >= 1)
		{
			$meta_keywords = func_get_args();

			$this->_meta_k  = implode($this->_title_separator, $meta_keywords);
		}
	...
	public function meta_description()
	{
		...
		if (func_num_args() >= 1)
		{
			$meta_description = func_get_args();
			$this->_meta_d = implode($meta_description);
		}
	...
	public function html_wf_page()
	{
		// If we have some segments passed
		if (func_num_args() >= 1)
		{
			$html_wf_page = func_get_args();
			$this->_h_page = implode($html_wf_page);
		}
	...
	public function html_wf_site()
	{
		// If we have some segments passed
		if (func_num_args() >= 1)
		{
			$html_wf_site = func_get_args();
			$this->_h_site = implode($html_wf_site);
		}
