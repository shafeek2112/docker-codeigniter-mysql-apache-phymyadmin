...
class User_reset_password_model extends CI_Model {
    ...
    public function verify_token($email, $token) {
        $this->db->select('users.username, rp.token, rp.date_added');
        $this->db->from('users');
        $this->db->join('users_recover_password rp', 'users.email = rp.email');
    ...
    public function delete_token_data($token) {
        $this->db->delete('users_recover_password', array('token' => $token));
