    ...
    $.fn.columnFilter = function (options) {
        ...
        function _fnGetColumnValues(oSettings, iColumn, bUnique, bFiltered, bIgnoreEmpty) {
            ...
            for (var i = 0, c = aiRows.length; i < c; i++) {
                iRow = aiRows[i];
                var aData = dTable.fnGetData(iRow);
        ...
        function _fnColumnIndex(iColumnIndex) {
            if (properties.bUseColVis)
                ...
                return dTable.fnSettings().oApi._fnVisibleToColumnIndex(dTable.fnSettings(), iColumnIndex);
            //return iColumnIndex;
            //return dTable.fnSettings().oApi._fnColumnIndexToVisible(dTable.fnSettings(), iColumnIndex);
        }

        function fnCreateInput(dTable, regex, smart, bIsNumber, iFilterLength, iMaxLenght) {
            var sCSSClass = "text_filter";
            if (bIsNumber)
                sCSSClass = "number_filter";

            label = label.replace(/(^\s*)|(\s*$)/g, "");
            var currentFilter = dTable.fnSettings().aoPreSearchCols[i].sSearch;
            ...
            var input = $('<input type="text" class="' + search_init + sCSSClass + ' form-control" value="' + inputvalue + '"/>');
            ...
            if (bIsNumber && !dTable.fnSettings().oFeatures.bServerSide) {
                input.keyup(function () {
                    /* Filter on the column all numbers that starts with the entered value */
                    dTable.fnFilter('^' + this.value, _fnColumnIndex(index), true, false); //Issue 37
                    fnOnFiltered();
                });
            } else {
                input.keyup(function () {
                    if (dTable.fnSettings().oFeatures.bServerSide && iFilterLength != 0) {
                        //If filter length is set in the server-side processing mode
                        //Check has the user entered at least iFilterLength new characters

                        var currentFilter = dTable.fnSettings().aoPreSearchCols[index].sSearch;
                    ...
                    dTable.fnFilter(this.value, _fnColumnIndex(index), regex, smart); //Issue 37
                    fnOnFiltered();
                });
            }
        ...
        function fnCreateRangeInput(dTable) {

            th.html(_fnRangeLabelPart(0));
            var sFromId = dTable.attr("id") + '_range_from_' + i;
            var from = $('<input type="text" class="number_range_filter" id="' + sFromId + '" rel="' + i + '"/>');
            th.append(from);
            th.append(_fnRangeLabelPart(1));
            var sToId = dTable.attr("id") + '_range_to_' + i;
            ...
            */
            ...
            dTable.dataTableExt.afnFiltering.push(
            function (oSettings, aData, iDataIndex) {
                if (dTable.attr("id") != oSettings.sTableId)
                    return true;
        ...
        function fnCreateDateRangeInput(dTable) {
            th.html(_fnRangeLabelPart(0));
            var sFromId = dTable.attr("id") + '_range_from_' + i;
